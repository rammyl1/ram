...
// ── Spawn timer config ─────────────────────────────────────────────────────
const VEHICLE_TYPES: VehicleType[] = ["car","car","bus","tricycle","truck","schoolbus","car","tricycle"];
const SPAWN_INTERVAL_MS = 9000;  // spawn a new vehicle every N ms (reduced traffic)
const MAX_VEHICLES      = 10;    // cap concurrent vehicles on map (was 20)
...
export default function PixelGame({ onExit }: { onExit: () => void }) {
  ...
  // Keep ref in sync
  useEffect(() => {
    minimapMinimizedRef.current = isMinimapMinimized;
  }, [isMinimapMinimized]);
  ...
  const getTargetCam = useCallback((px: number, py: number, W: number, H: number) => {
    ...
    return {
      // If viewport is bigger than map, center it; otherwise clamp normally
      x: mapW <= W ? (mapW - W) / 2 : Math.max(0, Math.min(tx, mapW - W)),
      y: mapH <= H ? (mapH - H) / 2 : Math.max(0, Math.min(ty, mapH - H)),
    };
  }, []);
  ...
  const spawnNewVehicle = useCallback(() => {
    ...
    // Cap concurrent vehicles to keep traffic light
  ...
  useEffect(() => {
    ...
    const loop = (ts: number) => {
      ...
      const zoom = Math.max(1, Math.min(W / (30 * TILE_SIZE), 2.5));
      ...
      const pixelDist = Math.abs(targetPx-ren.px) + Math.abs(targetPy-ren.py);
      ren.isMoving = log.moving || pixelDist > 0.5;
      ...
      const tCam = getTargetCam(ren.px, ren.py, scaledW, scaledH);
      ...
      const x1=x0+Math.ceil(scaledW/TILE_SIZE)+3;
      const y1=y0+Math.ceil(scaledH/TILE_SIZE)+3;
      for (let ty=y0; ty<y1; ty++) {
        for (let tx=x0; tx<x1; tx++) {
          const tile = (tx >= 0 && tx < MAP_COLS && ty >= 0 && ty < MAP_ROWS) ? map[ty][tx] : 0;
          drawTile(ctx, tile, tx, ty, tx*TILE_SIZE-cam.x, ty*TILE_SIZE-cam.y, gf);
        }
      }
      ...
      for (const npc of npcsRef.current) {
        ...
        if (ns>-64&&ns<scaledW+64&&nt>-64&&nt<scaledH+64) drawNPC(ctx, npc, cam, gf);
      }
  ...
  }, [getTargetCam, updateNPCs, tryStep, getPedPositions, spawnNewVehicle, activeSection, isMinimapMinimized]);
  ...
  return (
    <div className="relative w-full h-full overflow-hidden bg-black select-none">
      ...
      <button 
        ...
        className="absolute bottom-[150px] left-3 z-20 px-2 py-1 bg-black/80 border border-yellow-600/60 text-yellow-500 text-[10px] font-mono hover:bg-yellow-600/20"
      >
        {isMinimapMinimized ? "□ MAXIMIZE MAP" : "− MINIMIZE MAP"}
      </button>

      {/* Mobile Joystick */}
      <div className="absolute bottom-10 left-10 z-20 md:hidden">
        <div 
          className="relative w-32 h-32 bg-white/10 rounded-full border-2 border-white/20 touch-none"
          onTouchStart={handleJoystick}
          onTouchMove={handleJoystick}
          onTouchEnd={stopJoystick}
        >
          <div 
            className="absolute w-12 h-12 bg-white/40 rounded-full border-2 border-white/60 transition-transform duration-75"
            style={{ 
              left: "calc(50% - 24px)", 
              top: "calc(50% - 24px)",
