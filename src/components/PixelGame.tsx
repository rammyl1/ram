...
/**
 * Pick a "zoom" factor so the existing fixed-size map COVERS the entire
 * browser viewport instead of leaving empty space on wide monitors.
 *
 * We don't extend the map — we just enlarge each pixel-art pixel
 * proportionally to the resolution. The camera follows the player along
 * any axis where the viewport is shorter than the map.
 */
function computeRenderScale(viewportW: number, viewportH: number): number {
  const MAP_W_PX = MAP_COLS * TILE_SIZE;   // 30 * 32 = 960
  const MAP_H_PX = MAP_ROWS * TILE_SIZE;   // 30 * 32 = 960

  // FILL: scale large enough that the map covers both axes
  const fill = Math.max(viewportW / MAP_W_PX, viewportH / MAP_H_PX);

  // Clamp to keep things readable:
  //   min 1.0  → never smaller than native pixel size on any device
  //   max 4.0  → avoid super-zoom on giant ultrawide monitors
  return Math.max(1.0, Math.min(fill, 4.0));
}
...
export default function PixelGame({ onExit }: { onExit: () => void }) {
  ...
  // Mirror minimap state in a ref so the render loop can read the *current*
  // value without the effect having to re-mount (which would cause two RAF
  // loops to race and the minimap would still be drawn by the orphan loop).
  const minimapMinimizedRef = useRef(false);
  useEffect(() => { minimapMinimizedRef.current = minimapMinimized; }, [minimapMinimized]);
  ...
  // W/H here are LOGICAL pixels (after dividing physical viewport by render scale)
  const getTargetCam = useCallback((px: number, py: number, W: number, H: number) => {
    const mapWPx = MAP_COLS * TILE_SIZE;
    const mapHPx = MAP_ROWS * TILE_SIZE;
    const tx = px - W/2 + TILE_SIZE/2;
    const ty = py - H/2 + TILE_SIZE/2;
    // If the viewport is wider than the map, centre the map and stop scrolling
    const clampedX = W >= mapWPx ? (mapWPx - W) / 2 : Math.max(0, Math.min(tx, mapWPx - W));
    const clampedY = H >= mapHPx ? (mapHPx - H) / 2 : Math.max(0, Math.min(ty, mapHPx - H));
  ...
  useEffect(() => {
    ...
    const loop = (ts: number) => {
      ...
      // ── Responsive render scale ──
      // Physical viewport pixels:
      const PW = canvas.width, PH = canvas.height;
      const scale = computeRenderScale(PW, PH);
      // Logical pixels (the world-space viewport size after zoom)
      const W = PW / scale;
      const H = PH / scale;

      // ── Camera lerp (in logical pixels) ──
      ...
      // Vignette (still in scaled space — covers the world)
      const vig=ctx.createRadialGradient(W/2,H/2,H*0.22,W/2,H/2,H*0.9);
      vig.addColorStop(0,"rgba(0,0,0,0)");
      vig.addColorStop(1,"rgba(0,0,0,0.42)");
      ctx.fillStyle=vig; ctx.fillRect(0,0,W,H);

      // ── Reset transform: HUD elements (minimap) draw at NATIVE scale ──
      ctx.setTransform(1, 0, 0, 1, 0, 0);

      // ── Minimap (HUD layer) — read live ref so toggling works instantly ──
      if (!minimapMinimizedRef.current) {
        drawMinimapWithVehicles(ctx, map, log, vehiclesRef.current, PW, PH);
      }
  ...
  // ── Joystick → keyboard bridge ─────────────────────────────────────────
  // The joystick simulates a held key, so the existing key-repeat movement
  // system handles step rate, smooth lerp, and walk animation automatically.
  const joystickActiveDir = useRef<string | null>(null);
  const handleJoystick = useCallback((dir: "up" | "down" | "left" | "right" | null) => {
    const k = keysRef.current;
    // Clear previous joystick key
    if (joystickActiveDir.current) {
      const prev = joystickActiveDir.current;
      if (prev === "up")    k.delete("w");
      if (prev === "down")  k.delete("s");
      if (prev === "left")  k.delete("a");
      if (prev === "right") k.delete("d");
    }
    joystickActiveDir.current = dir;
    if (dir === "up")    k.add("w");
    if (dir === "down")  k.add("s");
    if (dir === "left")  k.add("a");
    if (dir === "right") k.add("d");
  }, []);
  ...
  return (
    <div className="relative w-full h-full overflow-hidden bg-black select-none">
      ...
      {/* ── Minimap minimize / restore button ── */}
      <button
        onClick={() => setMinimapMinimized(m => !m)}
        className="absolute z-20 font-mono text-xs font-bold transition-all"
        style={{
          left: minimapMinimized ? "12px" : "150px",
          bottom: minimapMinimized ? "12px" : (130 + 22) + "px",
          background: minimapMinimized ? "rgba(20,30,50,0.95)" : "rgba(5,10,20,0.92)",
          border: "2px solid rgba(200,160,30,0.7)",
          color: "#ffd700",
          padding: minimapMinimized ? "6px 12px" : "3px 8px",
          boxShadow: "2px 2px 0 rgba(0,0,0,0.6)",
          cursor: "pointer",
        }}
        title={minimapMinimized ? "Show map" : "Hide map"}
      >
        {minimapMinimized ? "🗺 SHOW MAP" : "─ HIDE"}
      </button>

      {/* ── Mobile / touch controls: joystick (left) + action buttons (right) ── */}
      <div className="absolute bottom-4 left-4 z-10 md:hidden">
        <Joystick onDirectionChange={handleJoystick} />
      </div>
      <div className="absolute bottom-4 right-4 z-10 md:hidden flex flex-col gap-2 items-center">
        <button
          onTouchStart={(e) => { e.preventDefault(); checkInteraction(); }}
          onClick={checkInteraction}
          className="font-mono font-bold text-sm text-green-300 flex items-center justify-center active:opacity-60 active:translate-y-[2px]"
          style={{
            width: 70, height: 70,
            background: "rgba(5,10,20,0.88)",
            border: "3px solid rgba(80,200,80,0.7)",
            borderRadius: "50%",
            boxShadow: "3px 3px 0 rgba(0,0,0,0.6)",
          }}
        >E / TALK</button>
        <button
          onTouchStart={(e) => { e.preventDefault(); setMinimapMinimized(m => !m); }}
          onClick={() => setMinimapMinimized(m => !m)}
          className="font-mono font-bold text-xs text-yellow-300 px-3 py-1.5"
          style={{
            background: "rgba(5,10,20,0.88)",
            border: "2px solid rgba(200,160,30,0.6)",
            boxShadow: "2px 2px 0 rgba(0,0,0,0.6)",
          }}
        >{minimapMinimized ? "🗺" : "─"}</button>
      </div>
