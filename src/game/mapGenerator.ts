...
export function generateMap(): number[][] {
  ...
  // ── ASPHALT ROADS — 2 lanes per road (was 3) ────────────────────────────
  for (let x = 0; x < MAP_COLS; x++) {
    map[10][x] = TILES.PATH_H;  // top road, lane 1 (right-bound)
    map[11][x] = TILES.PATH_H;  // top road, lane 2 (left-bound)
    map[21][x] = TILES.PATH_H;  // bottom road, lane 1
    map[22][x] = TILES.PATH_H;  // bottom road, lane 2
  }
  for (let y = 0; y < MAP_ROWS; y++) {
    map[y][9]  = TILES.PATH_V;  // left road, lane 1 (down-bound)
    map[y][10] = TILES.PATH_V;  // left road, lane 2 (up-bound)
    map[y][19] = TILES.PATH_V;  // right road, lane 1
    map[y][20] = TILES.PATH_V;  // right road, lane 2
  }
